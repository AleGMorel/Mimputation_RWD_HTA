###### Introduce missings at 50% in HY_HC complete datasets######
...
MISSING50 <- function(ds){
  ...
  missing_p_Y <- inv_logit(-4 + 0.01*ds$rom + 0.02*ds$age + 0.02*ds$depression) # probability of missing in outcome
  missing_p_cost <- inv_logit(-3 + 0.01*ds$rom + 0.02*ds$age + 0.02*ds$depression) #probability of missing in costs
  ...
  print(percentage_miss) #percentage of missing data in the dataset
...
missing50 <- lapply(data,MISSING50)

for (i in 1:20) {
  
  write_xlsx(missing50[[i]], paste0("C:/Users/aegue/Documents/HTA PhD/Missing Data Simulation/R codes/Time to Get Real/Data/HY_HC/MISSING50/M50_HL",i,".xlsx"))
}
